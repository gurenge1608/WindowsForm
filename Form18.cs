...
namespace WindowsForm
{
    public partial class Form18 : Form
    {
        ...
        void BindData()
        {
            SqlCommand cmd = new SqlCommand("select TOP 3 tacgiasangtac, NewsID, Tomtat, Tieude, Filebaocao, TACGIA_AuthorID, Chapnhan from (((BAIBAO JOIN BAIPHANBIEN ON BAIBAO_NewsID = NewsID) JOIN THUCHIENPHANBIEN ON BPBID =  BAIPHANBIEN_BPBID) JOIN NHAPHANBIEN ON NHAPHANBIEN_PBID = PBID) JOIN BAIBAO_TACGIASANGTAC ON BAIBAO_TACGIASANGTAC.BAIBAO_NewsID = BAIBAO.NewsID where NHAKHOAHOC_ScientistID = '"+res+"' AND Chapnhan = 1;", conn);
